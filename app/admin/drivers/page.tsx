      ...
      <div style={{ minHeight: '100vh', backgroundColor: '#f9fafb' }}>
        <header style={{ backgroundColor: '#1F3A5F', color: 'white', padding: '1rem 2rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
            ...
            <h1 style={{ margin: 0, fontSize: '1.25rem', fontWeight: 700 }}>🚛 Drivers</h1>
          </div>
        ...
        <div style={{ padding: '2rem' }}>
          ...
          {loading && <p style={{ color: '#6b7280' }}>Loading drivers...</p>}
          {error && <p style={{ color: '#dc2626' }}>Error: {error}</p>}
          {!loading && items.length === 0 && <p style={{ color: '#6b7280' }}>No drivers found.</p>}
          {items.map((item) => (
            <div key={String(item.id)} style={{ backgroundColor: 'white', border: '1px solid #e5e7eb', borderRadius: '8px', padding: '1rem 1.25rem', marginBottom: '0.75rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <span style={{ fontWeight: 700, color: '#1F3A5F' }}>{String(item.first_name)} {String(item.last_name)}</span>
                  <span style={{ marginLeft: '1rem', color: '#6b7280', fontSize: '0.875rem' }}>{String(item.phone ?? '')}</span>
              </div>
