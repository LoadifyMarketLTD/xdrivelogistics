...
export function KPIStats() {
  ...
  return (
    <section ref={sectionRef} style={{ backgroundColor: '#F4F6F8', padding: '5rem 0' }}>
      <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '0 24px' }}>
        <div style={{ textAlign: 'center', marginBottom: '3rem' }}>
          <h2 style={{ fontSize: 'clamp(1.75rem, 4vw, 2.5rem)', fontWeight: 800, color: '#1C1C1C', marginBottom: '0.75rem' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '2rem' }}>
          {STATS.map((stat, index) => (
            ...
            >
              ...
              <div style={{ fontSize: 'clamp(2.5rem, 5vw, 4rem)', fontWeight: 800, color: '#1F3A5F', lineHeight: '1', marginBottom: '0.5rem' }}>
                {stat.value}
              </div>
              <div style={{ width: visible ? '40px' : '0px', height: '3px', backgroundColor: '#2E7D32', borderRadius: '2px', margin: '0.75rem auto', transition: `width 0.6s ease ${index * 0.15 + 0.3}s` }} />
              <div style={{ fontSize: '1rem', fontWeight: 700, color: '#1C1C1C', marginBottom: '0.25rem' }}>{stat.label}</div>
              <div style={{ fontSize: '0.85rem', color: '#888' }}>{stat.description}</div>
            </div>
          ))}
        </div>
      </div>
