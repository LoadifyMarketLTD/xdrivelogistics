...
-- 2.12 job_bids
-- Note: amount/bid_price_gbp and bidder_user_id/bidder_id are kept as alias
-- column pairs for backwards compatibility with older code that may reference
-- either name.  The trg_sync_job_bid_price trigger (section 6) keeps each
-- pair in sync automatically.
CREATE TABLE IF NOT EXISTS public.job_bids (
  ...
  amount           numeric(12,2),   -- alias of bid_price_gbp
  bid_price_gbp    numeric(12,2),   -- alias of amount
  currency         text    DEFAULT 'GBP',
  message          text,
...
-- profiles
-- ADD COLUMN IF NOT EXISTS skips the whole definition (including any FK) when
-- the column already exists, making these blocks safe to re-run.
DO $$ BEGIN
...
-- job_bids (alias columns)
-- ADD COLUMN IF NOT EXISTS skips the whole definition (including FK) when the
-- column already exists, so re-running this block is safe.
DO $$ BEGIN
...
-- quotes (ensure all columns are present on older schema versions)
DO $$ BEGIN
...
AS $$
BEGIN
  -- Synchronise amount ↔ bid_price_gbp
  ...
  -- Synchronise bidder_id ↔ bidder_user_id
...
-- ──────────────────────────────────────────────────────────────
-- 9. RLS POLICIES (create only if absent)
-- ──────────────────────────────────────────────────────────────
DO $$
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_policies
                 WHERE schemaname = 'public' AND tablename = 'companies'
                   AND policyname = 'companies_select_member') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_policies
                 WHERE schemaname = 'public' AND tablename = 'company_memberships'
                   AND policyname = 'memberships_select_member') THEN
