...
-- and wait for it to complete successfully before running this.
-- 
-- This is the main migration that creates all tracking tables,
-- fields, functions, and policies.
-- ============================================================
...
BEGIN
  ...
  IF has_enum THEN
    -- Check if completed value exists in enum
    SELECT EXISTS (
    ...
    IF NOT has_completed THEN
      RAISE EXCEPTION 'ERROR: Enum value ''completed'' not found. Please run STEP-1-ENUM.sql first!';
    END IF;
    
    RAISE NOTICE '✅ Enum check passed: ''completed'' value exists';
    can_proceed := true;
  ELSE
    -- No enum, means TEXT column - that's fine
    RAISE NOTICE '✅ No enum type - using TEXT with CHECK constraint';
    can_proceed := true;
  END IF;
...
-- ============================================================
-- 1. FIX STATUS CHECK CONSTRAINT (for TEXT columns)
-- ============================================================
DO $$
DECLARE
  constraint_name TEXT;
BEGIN
  -- Only run if status is TEXT (not enum)
  ...
  ELSE
    RAISE NOTICE '✅ Status uses ENUM type - constraint not needed';
  END IF;
  
EXCEPTION
  WHEN OTHERS THEN
    RAISE NOTICE 'Note: Could not modify TEXT constraint (might not be needed)';
END $$;

-- ============================================================
-- 2. ADD TRACKING FIELDS TO JOBS TABLE
-- ============================================================
...
RAISE NOTICE '✅ Added tracking fields to jobs table';

-- ============================================================
-- 3. CREATE JOB_TRACKING_EVENTS TABLE
-- ============================================================
...
RAISE NOTICE '✅ Created job_tracking_events table';
...
RAISE NOTICE '✅ Created job_documents table';

-- ============================================================
-- 5. CREATE JOB_NOTES TABLE
-- ============================================================
...
RAISE NOTICE '✅ Created job_notes table';

-- ============================================================
-- 6. CREATE JOB_FEEDBACK TABLE
-- ============================================================
...
RAISE NOTICE '✅ Created job_feedback table';

-- ============================================================
-- 7. CREATE JOB_INVOICES TABLE
-- ============================================================
...
RAISE NOTICE '✅ Created job_invoices table';
...
-- ============================================================
-- 9. CREATE JOBS_WITH_TRACKING VIEW
-- ============================================================
...
RAISE NOTICE '✅ Created jobs_with_tracking view';
...
BEGIN
  ...
  RAISE NOTICE '   • 5 new tables: job_tracking_events, job_documents, job_notes, job_feedback, job_invoices';
  RAISE NOTICE '   • 60+ tracking fields in jobs table';
  RAISE NOTICE '   • 3 helper functions: add_tracking_event, update_job_pod, update_job_status';
  RAISE NOTICE '   • 1 view: jobs_with_tracking';
  RAISE NOTICE '   • Complete RLS policies for security';
  RAISE NOTICE '';
  RAISE NOTICE '🎉 Your tracking system is ready to use!';
END $$;
